...
// Chatbot AI Updated
var chatInputs = document.querySelectorAll('#chat input[type="text"], #chat textarea[name="messsage"]');
var chatBox = document.querySelector('.section-form-chat .chatbox');
var sendChatBtn = document.querySelector('#chat button');
var userMessage;
var API_KEY = "***";
...
var generateResponse = function(incomingChatLi) {
  ...
  fetch(API_URL, requestOptions).then(function(res) {
    ...
    incomingChatLi.querySelector('p').textContent = 'Error: ' + userMessage;
  });
};

var handleChat = function() {
  userMessage = Array.from(chatInputs).map(input => `${input.placeholder}: ${input.value}`).join(', ');
  Array.from(chatInputs).forEach(input => input.value = '');
